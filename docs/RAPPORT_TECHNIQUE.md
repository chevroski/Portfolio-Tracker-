...
- **Couverture** :  
  - `AssetTest` : quantités, prix moyen, total investi.  
  - `EncryptionServiceTest` : encrypt/decrypt round‑trip, gestion passphrase.  
...
- Cas d’erreur UI (dialogs, validations) couverts manuellement.
...
**Extrait : chargement asynchrone des données de chart (JavaFX Task)**  
Cet extrait montre l’exécution en arrière‑plan du calcul des séries, puis la mise à jour UI via `setOnSucceeded`.
...
> *Source :* `ChartController.loadChartData()` — `src/main/java/com/portfoliotracker/controller/ChartController.java`
